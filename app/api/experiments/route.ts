...
export async function GET() {
  try {
    ...
    const rows = await sql`
      SELECT id, title, created_by, current_level, status, experiment_type,
...
export async function POST(request: Request) {
  try {
    ...
    const sql = getDb();
    const rows = await sql`
      INSERT INTO experiment_designs (title, created_by)
      VALUES (${body.title || "Untitled Experiment"}, ${body.created_by || "anonymous"})
      RETURNING id, title, created_by, current_level, status, created_at
    `;
